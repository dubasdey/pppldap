...
static option_t ldap_options[] = {
        { "ldaphost", 		o_string, 	ldap_host,			"LDAP server host name",				OPT_PRIV | OPT_STATIC, NULL, (MAX_BUF - 1) },
        { "ldapdn", 		o_string, 	ldap_dn,			"DN to bind with to LDAP server",		OPT_PRIV | OPT_STATIC, NULL, (MAX_BUF - 1) },
        { "ldappw", 		o_string, 	ldap_pw,			"DN password",							OPT_PRIV | OPT_STATIC, NULL, (MAX_BUF - 1) },
        { "ldapport", 		o_int, 		&ldap_port,			"LDAP server port",						OPT_PRIV | OPT_STATIC },
        { "userbasedn", 	o_string,	userbasedn ,		"LDAP user base DN",					OPT_PRIV | OPT_STATIC, NULL, (MAX_BUF - 1) },
        { "ldapfilter", 	o_string, 	ldap_filter,		"LDAP filter"      ,					OPT_PRIV | OPT_STATIC, NULL, (MAX_BUF - 1) },
        { "ldaptimeout", 	o_int, 		&ldap_timeout,		"LDAP search timeout",					OPT_PRIV | OPT_STATIC },
        { "ldapnettimeout", o_int, 		&ldap_nettimeout,	"LDAP network activity timeout",		OPT_PRIV | OPT_STATIC },
        { "ldapusetls", 	o_bool, 	&ldap_usetls,		"Connect to LDAP server using TLS", 1 },
        { NULL }
};
...
static int ldap_address_allowed(u_int32_t addr) {
        ...
        if ((ipcp_wantoptions[0].hisaddr != 0) && (ipcp_wantoptions[0].hisaddr == addr)) {return 1;}
